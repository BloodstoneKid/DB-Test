...
const getClientes = async (req, res) => {
    const response = await pool.query('SELECT id_cliente, nombre, apellido FROM AMY_Cliente;');
    res.end(JSON.stringify(response.rows));
}
const getInstrumento = async (req, res) => {
    const id = parseInt(req.params.id);
    const response = await pool.query('SELECT id_instrumento, tipo FROM AMY_Instrumento WHERE AMY_Instrumento.cliente_id_cliente = $1;',[id]);
    res.end(JSON.stringify(response.rows));
}
